...
const middlewares = require('../middlewares/validDish');
const middlewaresUser = require('../middlewares/validUser');
...
router.get('/', (req, res) => {
  ...
  dataBase
    .query(dishes, { type: sequelize.QueryTypes.SELECT })
    .then((dishes) => {
...
router.post(
  '/',
  middlewaresUser.authenticateAdmin,
  middlewares.dishRepeat,
  (req, res) => {
    const query = 'INSERT INTO dishes (dish,price) VALUES (?,?)';
    const { dish, price } = req.body;
    dataBase
      .query(query, { replacements: [dish, price] })
      .then((response) => {
...
router.put(
  ...
  (req, res) => {
    ...
    const { dish, price } = req.body;
    dataBase
      .query(query, { replacements: [dish, price, id] })
      .then((response) => {

...
const middlewares = require('../middlewares/validUser');
...
router.post('/', middlewares.userRepeat, (req, res) => {
  const query =
    'INSERT INTO users (code_rol,username,email,name,phone,password,address) VALUES (1,?,?,?,?,?,?)';
  const { username, email, name, phone, password, address } = req.body;
  dataBase
    .query(query, {
      replacements: [username, email, name, phone, password, address],
    })
    .then((response) => {
...
router.put(
  '/',
  middlewares.authenticateUser,
  middlewares.userExist,
  (req, res) => {
    const { username } = req;
    const { email } = req;
    const query =
      'UPDATE users SET phone = ?,password = ?,address = ? WHERE username = ? OR email = ?';
    const { phone, password, address } = req.body;
    dataBase
      .query(query, {
        replacements: [phone, password, address, username, email],
      })
      .then((response) => {
...
router.delete(
  ...
  (req, res) => {
    ...
    dataBase
      .query(query, { replacements: [username, email] })
      .then(() => {
